...
function getKissGlobalPath() {
  if (Cache.KISS_IS_INSTALLED !== null) return Cache.KISS_IS_INSTALLED;
  try {
    ...
    const paths = ['node_modules', 'kiss'];
    if (!IS_WIN) paths.shift('lib');
    const kissDirectory = path.join(prefixPath, ...paths);
...
const Kiss = {
  ...
  insert() {
    ...
    if (!currentSavedFilepath) {
      const description = 'Atom Kiss Error';
      atom.notifications.addError(ERRORS.INLINE, { description });
      return;
    }
    ...
    const processOptions = {
      ...
      stdout: output => {
        ...
        const message = `Template ${userSelectedText} has been succesfully created`;
